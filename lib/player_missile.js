...
PlayerMissile.prototype.handleCollision = function () {
	...
	ParticleHelper.createExplosion(this.collision.pos, "#ff0000", options);
...
PlayerMissile.prototype.handleSeeking = function () {
	...
	var missileCenter = this.collision.getCenter();

	$.each(enemyObjects, function (id, enemyObj) {
		var enemyCenter = enemyObj.collision.getCenter();
		var distFromEnemy = missileCenter.distance(new Vector({ x: enemyCenter.x, y: enemyCenter.y }));
	...
	if(closestTarget) {
		...
		var newAddVel = closestTarget.collision.getCenter().subtract(missileCenter).normalize();
