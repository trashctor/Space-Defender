...
Engine.prototype.setupGame = function (twoPlayer) {
	...
	this.intervals.push(setInterval(this.spawnSpaceRock.bind(this), 1000));
	this.intervals.push(setInterval(this.spawnBackgroundParticle.bind(this), 100));
};
...
Engine.prototype.spawnBackgroundParticle = function () {
	...
	var colorDelta = { red: {min: 0, max: 25}, green: {min: -150, max: 50}, blue: {min: 0, max: 0} };
	var color = Util.randRGB(initialColor, colorDelta);

	var particle = new Particle(id, randPos, vel, radius, color, colorDelta);
